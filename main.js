...
const setTextContent = (id, textContent) => {
...
const setupBitsTable = () => {
    ...
    for (let i = 0; i < numBits; i++) {
        ...
        if (i == 0) {
            bitElem.classList.add('signbit');
            bitElem.title += ' (sign bit)';
        } else if (i < NumberProperties[currentFormat].exponentWidth + 1) {
            bitElem.classList.add('exponent');
            bitElem.title += ' (exponent)';
        } else {
            bitElem.classList.add('significand');
            bitElem.title += ' (significand)';
        }
    }
}
...
const reloadNumber = event => {
    ...
    const significandBitPatternOffset = 1 + prop.exponentWidth;
    const exponentBitPattern = new Uint8Array(rawBitPattern, 1, prop.exponentWidth);
    const significandBitPattern = new Uint8Array(rawBitPattern, significandBitPatternOffset, numBits - 1 - prop.exponentWidth);

    const exponentZeroOffset = (1 << (prop.exponentWidth - 1)) - 1;
    const exponentSum = sum(exponentBitPattern);
    const significandSum = sum(significandBitPattern);
    const implicitLeadingSignificandBit = exponentBitPattern.includes(1) ? 1 : 0;
    ...
    const f = [...Array(numBits - 1 - prop.exponentWidth).keys()].reduce((acc, i) => {
        return acc + Math.pow(2, -(i + 1)) * significandBitPattern[i];
    }, implicitLeadingSignificandBit);
    ...
    setTextContent('formula.sign_bit', fullBitPattern[0]);
    setTextContent('formula.exp_val', e);
    setTextContent('formula.exp_offset', exponentZeroOffset);
    setTextContent('formula.significand', f);
}
